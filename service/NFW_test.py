...
def rho_fit():
	# try to fit the density profile with NFW model
	load2 = 'NewMDCLUSTER_'
	#sub_set = CID ## with params range: ([13.95, 1], [14.95, 10.5])
	#sub_set = ['0080', '0099', '0106', '0118', '0119', '0132'] ## with params range: ([13.95, 1], [14.925, 6.25])
	#sub_set = ['0132'] ## with params range: ([13.95, 1], [15.05, 3.5])
	sub_set = ['0146']
	Nh = len(sub_set)
	for k in range(Nh):
		with h5py.File(
		...
		cc = fit_1[1]
		'''
		popt, pcov = curve_fit(rho_t, rh, rhoD, bounds = ([13.95, 1], [14.8, 9.95]))
		mc = popt[0]
		cc = popt[1]

		print(mc)
		print(cc)
		fit_line = rho_t(rh, mc, cc)
		...
		plt.xlim(1e0, 2*np.max(rh))
		plt.xscale('log')
		plt.xlabel(r'$R[kpc / h]$')
		plt.yscale('log')
		plt.text(5e0, 1e6, s = r'$M_{h} = %.2f$' % mc + '\n' + r'$C = %.2f$' % cc, fontsize = 'x-large')
