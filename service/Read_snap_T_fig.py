...
for k in range(len(z_mu)):
    try:
        with h5py.File('ratio_T_rho_h%.0f_z%.3f_MU.h5'%(_id_,z_mu[k])) as f:
            array1 = np.array(f['a'])
        ...
        plt.savefig('T_SFR/gas overdens_T distribution MU%.0f'%k,dpi = 600)
        plt.show() 
        plt.close()
        
        with h5py.File('rho_T_h%.0f_z%.3f_MU.h5'%(_id_,z_mu[k])) as f:    
            array2 = np.array(f['a'])
        ...
        plt.savefig('T_SFR/SFR-rho-gas-MU%.0f'%k,dpi=600)
        plt.show()
        plt.close()
        
        with h5py.File('main%.0f_halo_rho_T_z%.3f_MU.h5'%(_id_,z_mu[k])) as f:
            array3 = np.array(f['a'])
        ...
        plt.ylabel(r'$SFR$')
        plt.title(r'$SFR-\rho_{gas}-h%.0f-z_{%.3f}-MU$'%(_id_,z_mu[k]))
        plt.savefig('T_SFR/SFR rho_gas mhalo MU%.0f'%k,dpi=600)
        plt.show()
        plt.close()
        ...
        plt.savefig('T_SFR/gas density_T mhalo MU%.0f'%k,dpi = 600)
        plt.show()
        plt.close()
    except KeyError:
...
for k in range(len(z_gx)):
    try:
        with h5py.File('ratio_T_rho_h%.0f_z%.3f_GX.h5'%(_id_,z_gx[k])) as f:
        ...
        plt.savefig('T_SFR/gas overdens temperatur distribution GX%.0f'%k,dpi = 600)
        plt.show() 
        plt.close()
        
        with h5py.File('rho_T_h%.0f_z%.3f_GX.h5'%(_id_,z_gx[k])) as f:    
        ...
        plt.savefig('T_SFR/SFR-rho-gas-GX%.0f'%k,dpi=600)
        plt.show()
        plt.close()
        
        with h5py.File('main%.0f_halo_rho_T_z%.3f_GX.h5'%(_id_,z_gx[k])) as f:
        ...
        plt.ylabel(r'$SFR$')
        plt.title(r'$SFR-\rho_{gas}-h%.0f-z_{%.3f}-GX$'%(_id_,z_gx[k]))
        plt.savefig('T_SFR/SFR rho_gas mhalo GX%.0f'%k,dpi=600)
        plt.show()
        plt.close()
        ...
        plt.savefig('T_SFR/gas density_T mhalo GX%.0f'%k,dpi = 600)
        plt.show()
        plt.close() 
    except KeyError:
