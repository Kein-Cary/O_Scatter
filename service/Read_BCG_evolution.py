...
def BCG_evolution(ip,resolution,g_size):
    ...
    with h5py.File('BCG_%.0f_R_%.3f.h5'%(size_BCG,resolution),'w') as f:
        f['a'] = np.array(r)
    ...
    with h5py.File('BCG_%.0f_m_MU_%.3f.h5'%(size_BCG,resolution),'w') as f:
        f['a'] = np.array(m)
    ...
    with h5py.File('BCG_%.0f_rho_MU_%.3f.h5'%(size_BCG,resolution),'w') as f:
        f['a'] = np.array(rho)
    ...
    with h5py.File('BCG_%.0f_mrho_MU_%.3f.h5'%(size_BCG,resolution),'w') as f:
        f['a'] = np.array(m_rho)
    ...
    with h5py.File('BCG_%.0f_m_GX_%.3f.h5'%(size_BCG,resolution),'w') as f:
        f['a'] = np.array(m_gx)
    ...
    with h5py.File('BCG_%.0f_rho_GX_%.3f.h5'%(size_BCG,resolution),'w') as f:
        f['a'] = np.array(rho_gx)
    ...
    with h5py.File('BCG_%.0f_mrho_GX_%.3f.h5'%(size_BCG,resolution),'w') as f:
    ...
    z = np.array([z_mu,z_gx])
    with h5py.File('BCG_redshift.h5','w') as f:
        f['a'] = np.array(z)
    with h5py.File('BCG_redshift.h5') as f:
        for t in range(np.int0(z.shape[0])):
            f['a'][t,:] = np.array(z[t,:])
    return
