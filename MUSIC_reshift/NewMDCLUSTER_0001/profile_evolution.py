...
for q in range(l_rang):
    if q % 6 == 0:
        ...
        if _ix==0 :
            ...
            plt.plot(r[sum_bin:sum_bin+N],m_star[sum_bin:sum_bin+N],'--',
                     color = mpl.cm.rainbow(q/l_rang),label = r'$M_\ast -%.3f$'%goal_z)
            plt.plot(r[sum_bin:sum_bin+N],m_gas[sum_bin:sum_bin+N],'.-',
                     color = mpl.cm.rainbow(q/l_rang),label = r'$M_{gas} -%.3f$'%goal_z)
...
plt.xlim(1e0,1e3)
plt.ylim(1e8,1e16)
plt.savefig('mass evolution MUSIC',dpi = 600)
plt.show()
'''
...
plt.xlim(1e0,1e3)
plt.ylim(1e4,1e11)
plt.savefig('density evolution MUSIC',dpi = 600)
plt.show()
'''