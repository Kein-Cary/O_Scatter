...
main_value = asc.read('D:/mask/MUSIC/MUSIC_reshift/NewMDCLUSTER_0001/GadgetMUSIC-NewMDCLUSTER_0001.z0.000.AHF_mtree_idx',
...
with h5py.File('main_tree.h5','w') as f:
     f['a'] = np.array(main_tree)
for k in range(0,len(redshift)-1):
    if redshift[k] <= 9.75 :
        id_value = asc.read('D:/mask/MUSIC/MUSIC_reshift/NewMDCLUSTER_0001/GadgetMUSIC-NewMDCLUSTER_0001.z%.3f.AHF_mtree_idx'%redshift[k],
                    converters={'col1':[asc.convert_numpy(np.int64)], 'col2':[asc.convert_numpy(np.int64)]})
        if k == 0 :
...
with h5py.File('basic_tree.h5','w') as f:
