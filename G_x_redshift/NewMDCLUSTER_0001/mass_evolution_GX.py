...
l = len(redshift)
halo_list = asc.read('D:/mask/G_X/G_x_redshift/NewMDCLUSTER_0001/GadgetX-NewMDCLUSTER_0001.z0.000.AHF_halos',
                converters={'col1':[asc.convert_numpy(np.int64)], 'col2':[asc.convert_numpy(np.int64)]})
halo_id = np.array(halo_list['col1'])
_id_ = 0
goal_id = halo_id[0]#set the goal halo at z==0
...
for k in range(l):
    halo_read = asc.read('D:/mask/G_X/G_x_redshift/NewMDCLUSTER_0001/GadgetX-NewMDCLUSTER_0001.z%.3f.AHF_halos'%redshift[k],
                converters={'col1':[asc.convert_numpy(np.int64)], 'col2':[asc.convert_numpy(np.int64)]})
...
plt.yscale('log')
plt.title('Gadget_GX_mass')
#plt.savefig('mass_evolution_GX',dpi=600)
plt.show()
