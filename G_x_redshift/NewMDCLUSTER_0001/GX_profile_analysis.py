...
M_halo = asc.read('D:/mask/G_X/G_x_redshift/NewMDCLUSTER_0001/GadgetX-NewMDCLUSTER_0001.z0.000.AHF_halos',
                  converters={'col1':[asc.convert_numpy(np.int64)], 'col2':[asc.convert_numpy(np.int64)]})
...
def get_data(z_value,haloid):
    ...
    halo_list = asc.read('D:/mask/G_X/G_x_redshift/NewMDCLUSTER_0001/GadgetX-NewMDCLUSTER_0001.z%.3f.AHF_halos'%z,
                converters={'col1':[asc.convert_numpy(np.int64)], 'col2':[asc.convert_numpy(np.int64)]})
    ...
    with h5py.File('halo_ID_GX%.0f.%.3f.h5'%(tr_halo,z),'w') as f:
            f['a'] = np.array(ID_save)
    with h5py.File('halo_ID_GX%.0f.%.3f.h5'%(tr_halo,z)) as f:
        for t in range(len(ID_save)):
            f['a'][t,:] = ID_save[t,:]
            
    #get the rows of the array
    with h5py.File('halo_data_GX%.0f.%.3f.h5'%(tr_halo,z),'w') as f:
        f['a'] = np.array(ARRAY)
    with h5py.File('halo_data_GX%.0f.%.3f.h5'%(tr_halo,z)) as f:
        for t in range(M):
            f['a'][t,:] = ARRAY[t,:]
    halo_profile = pd.read_table('D:/mask/G_X/G_x_redshift/NewMDCLUSTER_0001/GadgetX-NewMDCLUSTER_0001.z%.3f.AHF_profiles'%z,dtype = np.float)
    ...
    N = BRRAY.shape[0]
    with h5py.File('profile_data_GX%.0f.%.3f.h5'%(tr_halo,z),'w') as f:
        f['a'] = np.array(BRRAY)
    with h5py.File('profile_data_GX%.0f.%.3f.h5'%(tr_halo,z)) as f:
        for t in range(N):
            f['a'][t,:] = BRRAY[t,:]
    return ARRAY,BRRAY
...
def ana_profile(z,h_id):
    ...
    r_shift = z
    #read the halo file
    with h5py.File('halo_data_GX%.0f.%.3f.h5'%(tr_halo,r_shift)) as f:
        y1 = f['a']
        ARRAY = np.array(y1)
    '''
    cNFW = ARRAY[0,:]
    ...
    with h5py.File('profile_data_GX%.0f.%.3f.h5'%(tr_halo,r_shift)) as f:
         y2 = f['a']
         BRRAY = np.array(y2)
    r = BRRAY[0,:]
    n_part = BRRAY[1,:]
